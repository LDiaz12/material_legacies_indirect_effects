...
metadata1$TREATMENT <- factor(metadata1$TREATMENT, levels = c("Control", "Algae_Dom", "Coral_Dom", "Rubble_Dom"))
...
endos_model <- lmer(mean_endos ~ TREATMENT + (1|CORAL_TANK_NUM) + (1|GENOTYPE), data= metadata1)
...
chla_plot <- metadata %>% 
  ...
  scale_color_manual(values = c("Control" = "blue", "Algae_Dom" = "darkgreen", "Coral_Dom" = "coral",
                                "Rubble_Dom" = "tan")) +
  geom_point(data = metadata, aes(x = TREATMENT, y = chla_norm), alpha = 0.25) +
  stat_summary(fun.y = mean, geom = "point", size = 3) + 
  stat_summary(fun.data = mean_sdl, geom = "errorbar", fun.args = list(mult = 1), width = 0.1) +
  theme_bw() +
  theme(axis.text.x = element_text(size = 13, angle = 30, hjust = 1),
...
chla_model <- lmer(chla_norm ~ TREATMENT + (1|CORAL_TANK_NUM) + (1|GENOTYPE), data = metadata)
...
chla_meanpH_plot <- metadata1 %>%
  ggplot(aes(x = pH_mean, y = mean_chl)) + 
...
chla_meanpH_model <- lm(mean_chl ~ pH_mean, data = metadata1)
check_model(chla_meanpH_model)
summary(chla_meanpH_model)

# regression of chl-a and mean DOC # 
chla_meanDOC_plot <- metadata1 %>%
  ggplot(aes(x = DOC_mean, y = mean_chl)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
chla_meanDOC_model <- lm(mean_chl ~ DOC_mean, data = metadata1)
...
endo_meanpH_plot <- metadata1 %>%
  ggplot(aes(x = pH_mean, y = mean_endos)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
endo_meanpH_model <- lm(endo_per_cm2 ~ pH_mean, data = metadata1)
check_model(endo_meanpH_model)
summary(endo_meanpH_model)

# regression of endos and mean DOC #
endo_meanDOC_plot <- metadata1 %>%
  ggplot(aes(x = DOC_mean, y = mean_endos)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
endo_meanDOC_model <- lm(mean_endos ~ DOC_mean, data = metadata1)
...
biomass_meanpH_plot <- metadata1 %>%
  ggplot(aes(x = pH_mean, y = mean_biomass)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
biomass_meanpH_model <- lm(mean_biomass ~ pH_mean, data = metadata1)
check_model(biomass_meanpH_model)
summary(biomass_meanpH_model)

# regression of tissue biomass and mean DOC # 
biomass_meanDOC_plot <- metadata1 %>%
  ggplot(aes(x = DOC_mean, y = mean_biomass)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
biomass_meanDOC_model <- lm(mean_biomass ~ DOC_mean, data = metadata1)
...
# R and mean pH #
R_meanpH_plot <- metadata1 %>%
  ggplot(aes(x = pH_mean, y = mean_R)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
R_meanpH_model <- lm(mean_R ~ pH_mean, data = metadata1)
check_model(R_meanpH_model)
summary(R_meanpH_model)
anova(R_meanpH_model)

# R and mean DOC # 
R_meanDOC_plot <- metadata1 %>%
  ggplot(aes(x = DOC_mean, y = mean_R)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
R_meanDOC_model <- lm(mean_R ~ DOC_mean, data = metadata1)
...
# GP and mean pH # 
GP_meanpH_plot <- metadata1 %>%
  ggplot(aes(x = pH_mean, y = mean_GP)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
GP_meanpH_model <- lm(mean_GP ~ pH_mean, data = metadata1)
check_model(GP_meanpH_model)
summary(GP_meanpH_model)

# GP and mean DOC # 
GP_meanDOC_plot <- metadata1 %>%
  ggplot(aes(x = DOC_mean, y = mean_GP)) + 
  geom_point(aes(color = TREATMENT)) + 
  geom_smooth(method = "lm", formula = y~x) + 
...
GP_meanDOC_model <- lm(mean_GP ~ DOC_mean, data = metadata1)
